  ...
  return (
    <div className="h-full">
      <div className="flex flex-col w-full overflow-x-hidden text-slate-700 dark:text-slate-50 bg-gray-50 dark:bg-primary-800">
        <aside
          className={`${sidebarCollapsed ? 'md:w-16' : 'md:w-64'} bg-primary-600 dark:bg-primary-700 transition-width duration-700 w-full top-0 md:fixed bottom-0 z-30 flex-shrink-0   overflow-y-auto  lg:block`}
        >
              ...
              <nav className="flex-1 mt-5 text-xs font-medium leading-5 md:text-sm ">
                <ul className="m-0 ml-0 list-none">
                  {sidebarItems.map((item, index) => (
                    <li key={item.title} className={` ${currentIndex === index ? 'bg-primary-600' : ''}`}>
                      {' '}
                      <Link
                        ...
                        to={item.path}
                      >
                        <span className={`${item.icon} text-lg block`} />
                          ...
                          {item.children.map((child) => (
                            <li>
                              ...
                              <Link
                                ...
                                to={child.path}
                              >
                                <span className="i-carbon-software-resource" />
        ...
        <div
          className={`w-full  relative transition-padding duration-700 pl-0 ${sidebarCollapsed ? 'md:pl-16 rtl:pl-0 md:rtl:pr-16' : 'rtl:pl-0 md:rtl:pr-64 md:pl-64'}`}
        >
          <header
            className={`${sidebarCollapsed ? 'md:pl-20 md:rtl:pr-20' : 'pl-0 rtl:pl-2 md:rtl:pr-40 md:rtl:pr-72 md:pl-72'} pr-0 pl-2 transition-padding duration-700 w-full md:fixed right-0 z-20 flex flex-col items-center justify-center md:justify-between  px-4 py-2 space-y-2 shadow-md  md:py-0 md:space-y-none md:h-16 md:flex-row bg-slate-100 dark:bg-primary-900`}
          >
            ...
            <div>
              <div className="relative">
                <span className="absolute inset-y-0 z-10 flex items-center pl-3 text-xl text-gray-500 i-carbon-search top-1/4 left-3" />
                <input
                  type="text"
                  placeholder="Type Something"
                  className="w-full min-w-[300px] py-3 pl-10 pr-4 text-gray-700 bg-white border rounded-md dark:bg-primary-900 dark:text-gray-300 dark:border-gray-500 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring"
                />
              </div>
            </div>
            <div className="flex items-center space-x-4 rtl:space-x-reverse text-slate-600 dark:text-white">
              ...
              <div className="flex h-8 bg-primary-700">
                <button
                  ...
                  className={`${i18n.language === 'en' ? 'bg-primary-600' : 'bg-transparent'} w-8 cursor-pointer outline-none border-0 text-white`}
                >
                  En
                </button>
                <button
                  onClick={() => changeLang('ar')}
                  className={`${i18n.language === 'ar' ? 'bg-primary-600' : 'bg-transparent'} w-8 cursor-pointer  outline-none border-0 text-white`}
                >
                  ع
                </button>
              </div>
